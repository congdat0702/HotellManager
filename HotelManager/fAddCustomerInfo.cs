...
namespace HotelManager
{
    ...
    {
        private static List<string> listIdCustomer;

        public static List<string> ListIdCustomer { get => listIdCustomer; set => listIdCustomer = value; }

        public fAddCustomerInfo()
        {
            InitializeComponent();
            LoadCustomerType();
            ListIdCustomer = new List<string>();
        }
       ...
       //tìm thông tin khách hàng theo IDCard
        ...
        public void AddIdCustomer(string idCustomer)
        {
            if(ListIdCustomer.Count!=0)
            {
                bool check = false;
                foreach (string item in ListIdCustomer)
                {
                    if (item == idCustomer)
                    {
                        check = true;
        ...
        private void btnAddCustomer_Click(object sender, EventArgs e)
        {
            if(txbFullName.Text!=string.Empty&&txbIDCard.Text!=string.Empty&&txbAddress.Text!=string.Empty&&cbNationality.Text!=string.Empty&&txbPhoneNumber.Text!=string.Empty)
            {
                if (!IsIdCardExists(txbIDCard.Text))
                {
                    ...
                    txtIDCustomer.Text =GetAutomaticID();
