...
namespace DAO
{
    public class CustomerDAO
    {
        ...
        public bool InsertCustomer(string id,string idCard, string name,string idCustomerType,DateTime dateofBirth,string address,int phonenumber,string sex,string nationality)
        {
            string query = "USP_InsertCustomer_  @ID , @idCard , @name , @idCustomerType , @dateOfBirth , @address , @phoneNumber , @sex , @nationality";
            return DataProvider.Instance.ExecuteNoneQuery(query,new object[] {id, idCard, name, idCustomerType, dateofBirth, address, phonenumber, sex, nationality })>0;
        }
    ...
    public   bool InsertCustomer(string id, string customerName, string idCustomerType, string idCard, string address, DateTime dateOfBirth, int phoneNumber, string sex, string nationality)
        {
            string query = "exec USP_InsertCustomer @ID , @customerName , @idCustomerType , @idCard , @address , @dateOfBirth , @phoneNumber , @sex , @nationality";
            int count = DataProvider.Instance.ExecuteNoneQuery(query, new object[] {id , customerName, idCustomerType, idCard, address, dateOfBirth, phoneNumber, sex, nationality });
            return count > 0;
        }
        public bool InsertCustomer(Customer customer)
        {
            return InsertCustomer(customer.Id , customer.Name, customer.IdCustomerType, customer.IdCard, customer.Address,
        ...
        public bool UpdateCustomer(string id,string name,string idCard,string idCustomerType, int phoneNumber,DateTime dateOfBirth,string address,string sex,string nationality)
        {
